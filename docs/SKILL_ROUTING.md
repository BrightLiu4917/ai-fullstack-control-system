...
- generic adapter 不可用且用户确认手写时，转入 `backend-java-springboot`，并按 API contract、DBA、TDD 和 release review 执行。
...
-> backend-java-springboot，如 generic adapter 尚未实现且用户确认生产级手写
