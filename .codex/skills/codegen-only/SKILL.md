---
name: codegen-only
description: 兼容入口。CRUD、脚手架、Controller、Service、Mapper/XML、DTO/VO 生成任务必须改用 codegen-java-springboot-crud 先识别 adapter；adapter 不可用时可确认后转入生产级手写实现。
---
...
- generic adapter 不可用时，可在用户确认后转入 `backend-java-springboot` 做生产级手写实现。
- 覆盖已有文件必须等待用户确认。
